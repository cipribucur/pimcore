...
class SnowboardLinkGenerator implements LinkGeneratorInterface
{
    /**
    ...
    /**
     * SnowboardLinkGenerator constructor.
     *
    ...
    {
        if (!($object instanceof SnowboardLocations)) {
            throw new \InvalidArgumentException('Given object is no Snowboard Location');
        }
        ...
        return ForceInheritance::run(function () use ($object, $params) {
            ...
            if($document && !$fullPath) {
                $fullPath = $document->getProperty('snowboard_default_document') ? substr($document->getProperty('snowboard_default_document')->getFullPath(), strlen($localeUrlPart)) : '';
            }

            return $this->pimcoreUrl->__invoke(
                [
                'title' => Text::toUrl($object->getTitle() ? $object->getTitle() : 'snowboard'),
                'snowboard' => $object->getId(),
                'path' => $fullPath
            ],
                'snowboard-detail',
                true
